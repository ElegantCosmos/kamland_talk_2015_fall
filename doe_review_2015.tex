...
\begin{frame}{Test algorithm against $\mu$ (Data)}
	\begin{columns}[T]
		...
		\begin{block}{\centering{Cosmic ray $\mu$}}
			\vspace{5mm}
			\includegraphics[width=\linewidth]{through_going_muon-pmt_angle.pdf}
		\end{block}
		\column{0.5\linewidth}
...
\begin{frame}{Test algorithm against $\Pnu$ (MC)}
	...
	\begin{itemize}
		\item[] Legend:
		\begin{itemize}
			\item[] --- \SI{1}{\sigma} of reconstructed angle from \Pnu
				direction
			\item[] {\color{red}---} \SI{1}{\sigma} of lepton angle from \Pnu
				direction
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Data selection}
	\begin{itemize}
		...
		\item Event selection criteria:
			\begin{itemize}
				\item<2-> Fully contained events
					\begin{itemize}
						\item[]<3-> $\implies$ Outer detector PMT hits $< 5$
					\end{itemize}
				\item<4-> $E_{\text{reconstructed}} > \SI{1}{GeV}$ (predicted by
					theory)
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Reconstructed Vertex}
	...
	\begin{itemize}
		\item[] Legend:
			\begin{itemize}
				\item[] --- \SI{6.5}{\meter} radius balloon edge
				\item[] {\color{red}---} \SI{5.2}{\meter} radius fiducial volume
					cut
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{figure}
		\centering
		\begin{figure}
			\includegraphics[width=\linewidth]{chi2t_vs_radius.pdf}
		\end{figure}
	\end{figure}
\end{frame}
...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\item \textbf{First physics application} of $\Pnu$ directionality in
			scintillator.
	\end{itemize}
\end{frame}
