...
\begin{frame}{Motivation}
	\begin{itemize}
		\item<1-> Past analysis in KamLAND:\\
			\begin{itemize}
				\item[] mainly $\sim\!\si{\mega\electronvolt}$ \APnue (inverse-beta decay)
			\end{itemize}
		\item<2-> {\color{red}No analysis at higher
			$\sim\!\si{\giga\electronvolt}$ energies}
		\item<3-> {\color{red}Can we do \Pnu directionality?}
		\item<4-> {\color{red}Can we do \Pnu flavor discrimination?}
	\end{itemize}
\end{frame}
...
\begin{frame}{Test Hellgartner on T2K events (data)} {($E_{\text{reconstructed}} = \SI{363}{\mega\electronvolt}$)}
	\centering
...
\begin{frame}{High energy ($\gtrsim \si{\giga\electronvolt}$) calibration}
	{
		pe/MeV of scintillator, and comparison to Kat energy fitter\\
		$E_{\text{high}}$: high energy reconstruction\\
		$E_{\text{low}}$: Kat energy
	}
	\centering
	\includegraphics[width=0.8\linewidth]{scintID_corrected_allRuns.pdf}\\
	\includegraphics[width=0.8\linewidth]{high_vs_low_energy_recon_energy.pdf}
\end{frame}
...
\begin{frame}
	...
	%\end{tikzpicture}
	\begin{itemize}
		\item<2-> Reject events with $\chi_{\text{time}}^2 \geq 40$ that
			are too elongated in shape (most likely $\mu$)
	\end{itemize}
\end{frame}
